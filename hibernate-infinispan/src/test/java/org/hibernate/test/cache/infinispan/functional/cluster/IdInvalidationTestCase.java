...
import org.hibernate.test.cache.infinispan.functional.IdOnManyToOne;
 ...
 * @since // TODO
 */
public class IdInvalidationTestCase extends DualNodeTestCase {

   private static final Log log = LogFactory.getLog(IdInvalidationTestCase.class);
   ...
   private static AtomicBoolean touched = new AtomicBoolean(false);
	...
	private static Integer cPK1= new Integer(87);
	private static Integer cPK2= new Integer(88);
   @Override
   ...
   public void testAll() throws Exception {
      ...
      Cache localCache = localManager.getCache(Citizen.class.getName());
      ...
      Cache remoteCache = remoteManager.getCache(Citizen.class.getName());
...
//         remoteStats.logSummary();
         ...
         withTx(localTM, new Callable<Void>() {
            ...
            public Void call() throws Exception {
               ...
               s.beginTransaction();
               s.createQuery( "delete IdOnManyToOne" ).executeUpdate();
   ...
   private void assertLoadedFromCache(MyListener localListener, Integer id) {
	   String res = "";
	   if (!(touched.get())){
   ...
   private void getCitizenWithCriteria(TransactionManager tm, final SessionFactory sf) throws Exception {
      withTx(tm, new Callable<Void >() {
         ...
         public Void call() throws Exception {
            Session s = sf.openSession();
            Transaction tx = s.beginTransaction();
            Integer cPK = new Integer(87);
			Object on = s.get( Citizen.class, cPK1);
   ...
   private void changeCitizen(TransactionManager tm, final SessionFactory sf) throws Exception {
      withTx(tm, new Callable<Void >() {
         ...
         public Void call() throws Exception {
            ...
            criteria.list();*/
			Citizen cit = (Citizen) s.get( Citizen.class, cPK1);
			cit.setSsn( "2345");
			s.update(cit);
            // cleanup
            tx.commit();
