...
public abstract class AbstractSaveEventListener<C> implements CallbackRegistryConsumer {
	...
	private static EntityKey entityKey(Object id, EntityPersister persister, EventSource source) {
		...
		final Object old = persistenceContext.getEntity( persister, key );
		if ( old != null ) {
		...
		else if ( persistenceContext.containsDeletedUnloadedEntityKey( persister, key ) ) {
			source.forceFlush( persister, key );
		}
		return key;
	}
	...
	protected Object performSaveOrReplicate(
		...
		if ( original.getLoadedState() != null ) {
			persistenceContext.getEntityHolder( persister, key ).setEntityEntry( original );
		}
