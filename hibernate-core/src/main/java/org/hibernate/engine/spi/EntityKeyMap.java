...
public class EntityKeyMap<V> {
	...
	static class Node<V> {
		 ...
		 * The returned key carries the persister for convenience.
		 */
	...
	transient Node<V> @Nullable[] table;
	transient int size;
	...
	public @Nullable V put(EntityPersister persister, EntityKey key, V value) {
		...
		final Object id = key.getIdentifier();
		final Object csId = key.getChangesetId();
		return putVal( hash( rootName, id, csId ), rootName, id, csId, persister, value, false );
	}
	...
	public @Nullable V putIfAbsent(EntityPersister persister, EntityKey key, V value) {
		...
		final Object id = key.getIdentifier();
		final Object csId = key.getChangesetId();
		return putVal( hash( rootName, id, csId ), rootName, id, csId, persister, value, true );
	}
	...
	@Nullable Node<V> getNode(int hash, String rootEntityName,
		...
		final Node<V> @Nullable[] tab = table;
		if ( tab == null ) {
			return null;
		}
	...
	@Nullable V putVal(int hash, String rootEntityName, Object identifier,
		...
		if ( p == null ) {
			...
			tab[i] = new Node<>( hash, rootEntityName, identifier, changesetId, persister, value, null );
		}
		else {
			Node<V> e = null;
			// --- CUSTOM equality check (replaces key.equals) ---
			if ( keysEqual( p, hash, rootEntityName, identifier, changesetId ) ) {
				...
				for ( int binCount = 0; ; ++binCount ) {
					if ( ( e = p.next ) == null ) {
						// --- CUSTOM: Node created with key-part fields ---
						p.next = new Node<>( hash, rootEntityName, identifier, changesetId, persister, value, null );
						break;
					}
	...
	Node<V>[] resize() {
		final Node<V> @Nullable[] oldTab = table;
			...
			for ( int j = 0; j < oldCap; ++j ) {
				...
				if ( ( e = oldTab[j] ) != null ) {
					...
					if ( e.next == null ) {
						...
						@Nullable Node<V> loHead = null, loTail = null;
						@Nullable Node<V> hiHead = null, hiTail = null;
						@Nullable Node<V> next;
						do {
							next = e.next;
							if ( ( e.hash & oldCap ) == 0 ) {
	...
	abstract class HashIterator {
		...
		public final void remove() {
				final @Nullable Node<V> p = current;
			if ( p == null ) {
